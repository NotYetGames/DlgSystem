...
void UDialogueK2Node_Select::AllocateDefaultPins()
{
	...
	// Create the variable name pin, the one the selections are based on
	{
		...
		FDialogueBlueprintUtilities::SetPinAutogeneratedDefaultValueBasedOnType(VariableNamePin);
	}
	...
	// Create the default value pin
	{
		...
		FDialogueBlueprintUtilities::SetPinAutogeneratedDefaultValueBasedOnType(DefaultPin);
	}
	...
	for (const FName& PinName : PinNames)
	{
		...
		FDialogueBlueprintUtilities::SetPinAutogeneratedDefaultValueBasedOnType(NewPin);
	}
...
void UDialogueK2Node_Select::PinTypeChanged(UEdGraphPin* Pin)
{
	...
	if (Pin != GetVariableNamePin())
	{
		...
		for (UEdGraphPin* OptionPin : OptionPins)
		{
			...
			{
				FDialogueBlueprintUtilities::ResetPinToAutogeneratedDefaultValue(OptionPin);
			}
		}
